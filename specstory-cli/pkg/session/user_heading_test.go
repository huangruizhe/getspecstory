...
func TestRenderRoleHeader_UserIsColoredHeading(t *testing.T) {
	user := Message{Role: "user", Timestamp: "2026-06-19T05:04:33Z"}
	got := renderRoleHeader(user, true)

	if !strings.HasPrefix(got, "### ") {
		t.Errorf("user header should be a level-3 heading, got %q", got)
	}
	if !strings.Contains(got, "👤 User") {
		t.Errorf("user header should carry the 👤 marker, got %q", got)
	}
	// Plain Markdown only — no inline HTML, which some readers (Quick Look) leak
	// as a stray bracket instead of hiding.
	if strings.Contains(got, "<") || strings.Contains(got, ">") {
		t.Errorf("user header should be plain Markdown (no HTML tags), got %q", got)
	}
